...
const callAIforChatting = async (user_mssg,history = []) => {
  // In the new SDK, systemInstruction and config are passed within the call
  const formattedConversation = history.map(msg => ({
    role: msg.role,
  ...
  const result = await ai.models.generateContent({
    ...
    config: {
      ...
      Respond according to previous conversation history and user tone.
      If no context is given, respond in a friendly manner considering a new chat initiated.`,
    }
  });
  const text =
